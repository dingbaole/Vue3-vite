  ...
  <div class="compute-config" v-loading="resetLoading">
    ...
    <div class="config-item">
      ...
      <div class="button-group">
        <el-button
          v-permission="'excitation:rerun-company-goods-rule:add-goods-rule'"
          type="primary"
          @click="add('basic')"
        >
          新建激励草稿基础规则
        ...
        <el-button
          v-permission="'excitation:rerun-company-goods-rule:add-overlay-rule'"
          type="primary"
          plain
          @click="add('overlay')"
        >
        ...
        <el-button
          v-permission="'excitation:rerun-company-goods-rule:reset-goods-rule'"
          type="danger"
          plain
          @click="reset"
        >
        ...
        <el-table-column label="操作" width="120" align="center">
          <template #default="{ row }: { row: TableItem }">
            <el-button
              ...
              v-permission="
                'excitation:rerun-company-goods-rule:edit-goods-rule'
              "
              type="text"
              @click="edit(row)"
            >
            ...
            <el-button
              ...
              v-permission="
                'excitation:rerun-company-goods-rule:edit-overlay-rule'
              "
              type="text"
              @click="edit(row)"
            >
            ...
            <el-button
              v-permission="
                'excitation:rerun-company-goods-rule:delete-goods-rule'
              "
              type="text"
              @click="remove(row.draftCompanyGoodsExcitationRuleId)"
            >
              删除
            </el-button>
          </template>
...
const edit = (item: TableItem) => {
  if (item.ruleGroup === RuleGroup.basic) {
    GoodsRuleRef?.value.toggleDialogStatus(
      item.draftCompanyGoodsExcitationRuleId,
...
const remove = (draftCompanyGoodsExcitationRuleId: number) => {
  DeleteGoodsRule({
