...
import Tips from '@/views/goods/components/tips.vue'
  ...
  return customRef((track, trigger) => {
    return {
      ...
      set(newValue) {
        if (
          formData.operGoodsDetailInputList.length > 0 &&
          !formData.draftCompanyGoodsExcitationRuleId
        ) {
      ...
      .catch((action: Action) => {
        if (action === 'cancel') {
          const invalidation = hasInvalidation.map(
            ({ draftCompanyGoodsExcitationRuleId, goodsId, skuNo }) => {
              return {
...
const marketShopIdChange = (marketShopId?: number) => {
