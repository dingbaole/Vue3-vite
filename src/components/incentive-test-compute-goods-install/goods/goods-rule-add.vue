<template>
  <el-dialog
    v-model="dialogShow"
    :title="`${
      formData.draftCompanyInstallExcitationRuleId ? '编辑' : '新建'
    }安装激励草稿规则`"
    fullscreen
    :show-close="false"
  ...
  >
    <template #header>
      ...
      >
        ...
        <!-- <GoodsDesc v-if="!formData.draftCompanyInstallExcitationRuleId" /> -->
      </div>
    </template>
...
import type { OperGoodsDetailInput, ISaveBaseRule } from './api'
...
const selectDisabled = computed(() => {
  return !!formData.draftCompanyInstallExcitationRuleId || props.status?.cityId
})
  ...
  return customRef((track, trigger) => {
    return {
      ...
      set(newValue) {
        if (
          formData.operGoodsDetailInputList.length > 0 &&
          !formData.draftCompanyInstallExcitationRuleId
        ) {
          ...
          value = newValue
          formData.cityId = newValue
          if (!formData.draftCompanyInstallExcitationRuleId && newValue) {
            cityIdChange(newValue)
          }
        }
      }
      ...
      .catch((action: Action) => {
        if (action === 'cancel') {
          const invalidation = hasInvalidation.map(
            ({ draftCompanyInstallExcitationRuleId, materialNo, skuNo }) => {
              return {
...
const returnSaveApi = () => {
  return formData.draftCompanyInstallExcitationRuleId
    ? EditSaveBaseRule
    : AddSaveBaseRule
}
...
const init = () => {
  setTimeout(() => {
    ...
    formData.draftCompanyInstallExcitationRuleId = undefined
...
const initEdit = (companyId: number, marketShopId: number) => {
  const { draftCompanyInstallExcitationRuleId } = formData
  QueryBaseRule({
    draftCompanyInstallExcitationRuleId: draftCompanyInstallExcitationRuleId!,
    companyId,
    marketShopId
  }).then(res => {
    if (res.code === 200) {
      ...
      formData.companyInstallExcitationRuleId =
        res.data.companyInstallExcitationRuleId
...
const getMarketShopList = (companyId: number) => {
  MarketShopList({
    ...
    if (res.code === 200) {
      ...
      if (res.data?.length > 0 && props.status.marketShopId) {
        marketShopIdChange(props.status.marketShopId)
      }
    }
  })
}
...
watch([() => formData.commissionType, () => formData.commissionValue], () => {
  // if (formData.draftCompanyInstallExcitationRuleId) return
...
const toggleDialogStatus = (
  ...
  if (!dialogShow.value) {
    // 打开操作
    if (draftCompanyInstallExcitationRuleId && companyId) {
      // 编辑操作
      formData.draftCompanyInstallExcitationRuleId =
        draftCompanyInstallExcitationRuleId
