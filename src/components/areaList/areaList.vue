  ...
  returnApi()({
    ...
    if (res.code === 200) {
      ...
      pageData.tableData = mountNode(
        stationInfoList,
        filterAreaList(serviceAreaList)
      )
      checkedKeys.value = valueToRichValue(pageData.resData.serviceAreaList)
    }
  })
}
...
const valueToRichValue = (serviceAreaList: Area[]) => {
  return serviceAreaList
    .filter(item => allCheckKeys.value.has(item.serviceAreaId))
    .map(item => `${item.stationId}-${item.serviceAreaId}`)
}
...
const filterAreaList = (areaList: Area[]) => {
...
const mountNode = (stationList: Station[], empList: Area[]) => {
  ...
  empList.forEach(item => {
    const element = {
      ...
      richValue: `${item.stationId}-${item.serviceAreaId}`,
      label: `${item.areaName}`,
      ...item
    }
...
const confirm = () => {
  emit('onSelected', genSelectedAreaList())
  toggleDialogStatus()
}
