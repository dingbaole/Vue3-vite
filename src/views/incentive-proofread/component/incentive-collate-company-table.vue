<template>
  <div class="incentive-collate-company-table">
    ...
    <div class="form-wrap">
      <el-form :inline="true" :model="formData" ref="ruleFormRef">
        ...
        <el-form-item>
          <el-button v-permission="'excitation:order:settle-company-check:list'" type="primary" @click="search">查询</el-button>
          <el-button v-permission="'excitation:order:settle-company-check:list'" @click="reset">重置</el-button>
        </el-form-item>
      </el-form>
    </div>
      ...
      <div class="button-group">
        <el-button
          v-permission="'excitation:order:settle-company-check:asyn-export'"
          v-if="auditResult === AuditResult.WAIT_CHECK"
          type="primary"
          plain
        ...
        <el-button
          v-permission="'excitation:order:settle-company-check:problem-markers'"
          type="warning"
        ...
        <el-button
          v-permission="'excitation:order:settle-company-check:confirm-check'"
          v-if="auditResult === AuditResult.WAIT_CHECK"
          type="primary"
          :disabled="currentSelectedList.length === 0"
        ...
        <el-button
          v-permission="'excitation:order:settle-company-check:import'"
          v-if="auditResult === AuditResult.WAIT_CHECK "
      ...
      >
        ...
        <el-table-column prop="orderFinishTime" label="订单完成时间" align="center" width="160" date="YYYY-MM-DD HH:mm:ss" />
...
import ProblemMarkersCompanyC from './problemMarkersCompany.vue'
...
const formData = reactive({}) as Partial<Pick<CompanyCheck, 'orderNo' | 'relateOrderNo' | 'promoterId'>>
const refreshFlag = ref(0)
const { selectRemote, remoteMethod } = useRemoteSearchPromoter()

const roleIds = computed<RoleID>(() => {
  const {
    companyMonthStatementExcitationCheckId,
    companyMonthStatementExcitationCheckRoleId,
    companyMonthStatementExcitationCheckRoleUserRelationId
  } = props.initData
  return {
    companyMonthStatementExcitationCheckId,
    companyMonthStatementExcitationCheckRoleId,
...
type Table = API.ReturnFetchDataType<typeof GetListCompany>['list'][0]
...
const checkOrderExcitationItemInputList = computed(() => {
  return currentSelectedList.value.map(item => {
    return {
      ...
      auditRelationId: item.auditRelationId
    }
  })
})
 ...
 * @description 激励问题标记
 */
const dialogMarker = useDialog<typeof ProblemMarkersCompanyC>(ProblemMarkersCompanyC, { title: '激励问题标记' })
const openMarker = () => {
  dialogMarker.open({
...
.incentive-collate-company-table {
  .form-wrap {
    .el-input {
      width: 180px;
    }
