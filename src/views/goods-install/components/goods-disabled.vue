...
import { ref } from 'vue'
import { CloseServiceItemRule } from '@/webapi/goods-install'
import type { IViewExcitationGoods } from '@/webapi/goods-install'
  ...
  const goodsList = selectedList.value
    .map(item => {
      const {
        ...
        materialNo,
        materialId,
        companyInstallExcitationRuleMaterialRelationId
      } = item
      const list = item.excitationRuleOutputs
        .filter((item: any) => item.checked)
        .map(({ companyInstallExcitationRuleId, ruleName }: any) => {
          return {
            companyId,
            goodsId,
            materialId,
