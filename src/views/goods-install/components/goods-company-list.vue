<template>
  <el-dialog
    v-model="dialogShow"
    :title="`添加${mode === 'overlay' ? '指定' : ''}激励安装商品`"
    width="90%"
...
import { CanusedlGoodsList, MarketGoodsList } from '@/webapi/goods-install'
import type { OperGoodsDetailInput } from '@/webapi/goods-install'
...
const rowSelection = ref({
  ...
  getCheckboxProps: (record: any) => ({
    disabled:
      ...
      (record?.isLeafNode && record?.materialNo.search(',') !== -1)
  }),
  onSelect: (
...
const confirm = () => {
  const result = pageData.resData.goodsInfoList.filter(item =>
    allCheckKeys.value.has(item.materialNo)
  )
...
const init = () => {
  ...
  operGoodsDetailInputList.value.forEach(item =>
    allCheckKeys.value.add(item.materialNo)
  )
...
const materialNoChange = () => {
  getMarketGoodsList()
}
const goodsIdChange = (goodsId: number) => {
  const goods = materialOptions.value.filter(
    item => item.goodsId === goodsId
  )[0]
  materialNo.value = goods?.materialNo
}
...
const getMarketGoodsList = () => {
  MarketGoodsList({
    ...
    materialNo: materialNo.value ? materialNo.value : undefined
  }).then(res => {
    if (res.code === 200) {
 ...
 * @param data
 */
const mapState = function <T extends { value: number | string }>(data: T[]) {
  const _data: T[] = deepClone(data)
  const _map: Map<number | string, T> = new Map()
...
const allCheckKeys = ref<Set<number | string>>(new Set())
...
const filterHandler = (cate: Cate[], goods: Goods[]) => {
  ...
  const filterGoods = goods
    ...
    .filter(item => {
      if (materialNo.value) {
        return item.materialNo === materialNo.value
      }
      return true
    })
  return {
