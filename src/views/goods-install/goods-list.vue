<template>
  <el-card shadow="never">
    <el-form :inline="true" :model="formData" :rules="rules" ref="ruleFormRef">
      ...
      <el-form-item label="物料信息" prop="materialNo">
        <el-input
  ...
  <el-card shadow="never" style="margin-top: 10px">
    ...
    >
      <span>
        查询结果: 该店铺已设置激励安装商品数: {{ pageData.configuredGoodsNum }},
        未设置数:
    ...
    >
      ...
      <el-table-column label="安装商品激励规则" width="420">
        <template #default="scope">
          <div>
            <div>基础规则:</div>
            <div
            ...
            >
              <div v-if="item.ladderType === 11">
                <span
                  ...
                  :key="tiered.companyInstallExcitationRuleId"
                >
                  {{ tiered.ruleDesc }}
                </span>
              </div>
...
const getGoodsList = (marketShopId: number | null, materialNo?: string) => {
  MarketGoodsList({
    marketShopId,
    materialNo: materialNo ? materialNo : undefined
  }).then(res => {
    if (res.code === 200) {
...
const materialNoChange = (materialNo: string) => {
  goodsOptions.value = []
  formData.goodsId = undefined
  if (formData.marketShopId) {
    getGoodsList(formData.marketShopId, materialNo)
  }
}
const goodsIdChange = (goodsId: number) => {
  const goods = goodsOptions.value.filter(item => item.goodsId === goodsId)[0]
  if (goods) {
    formData.materialNo = goods.materialNo
  }
}
</script>
