<template>
  <el-dialog
    v-model="dialogShow"
    :title="`${
      formData.comnanyGoodsExcitationRuleId ? '编辑' : '新建'
    }激励基础规则`"
    fullscreen
    :show-close="false"
  ...
  >
    <template #header>
      ...
      >
        ...
        <GoodsDesc v-if="!formData.comnanyGoodsExcitationRuleId" />
      </div>
    </template>
    <el-form :inline="true" :model="formData" :rules="rules" ref="ruleFormRef">
      <div class="module-box">
        <el-alert
          ...
          title="规则名称建议命名格式：公司商品+“激励类型”+激励标准+激励规则。最大长度30个文字。"
          type="warning"
          :closable="false"
        />
      ...
      <div class="module-box">
        ...
        <el-form-item label="城市" prop="cityId">
          <el-select
            :disabled="!!formData.comnanyGoodsExcitationRuleId"
      ...
      <div class="module-box">
        ...
        <el-form-item v-if="formData.commissionType">
          <!-- <el-checkbox
          ...
          <div
            v-if="formData.commissionType === 12 && ladderTypeCustomRef === 11"
            style="display: flex; align-items: center"
          >
            <el-checkbox
              v-model="formData.ladderCalcType"
              :true-label="12"
              :false-label="11"
              label="超出阶梯累加计算"
            />
            <Tips style="margin-left: 8px" />
          </div>
        </el-form-item>
      </div>
      <div>
...
import type { OperGoodsDetailInput, ISaveBaseRule } from '@/webapi/goods'
    ...
    return {
      ...
      set(newValue) {
        if (
          ...
          value = newValue
          formData.cityId = newValue
          if (!formData.comnanyGoodsExcitationRuleId && newValue) {
            cityIdChange(newValue)
          }
        }
      }
...
const handleSaveError = (hasInvalidation: any[]) => {
  ...
  if (hasInvalidation.length > 0) {
    ElMessageBox.confirm(
      ...
      .then(() => {
        // 忽略继续提交, 移除失效的继续添加
        const hasInvalidaIds = hasInvalidation.map(item => item.goodsId)
        formData.operGoodsDetailInputList =
          formData.operGoodsDetailInputList.filter(
            item => !hasInvalidaIds.includes(item.goodsId)
          )
      ...
      .catch((action: Action) => {
        if (action === 'cancel') {
          const invalidation = hasInvalidation.map(
            ({ comnanyGoodsExcitationRuleId, goodsId, skuNo }) => {
              return {
...
const returnSaveApi = () => {
  return formData.comnanyGoodsExcitationRuleId
    ? EditSaveBaseRule
    : AddSaveBaseRule
}
...
const initEdit = (companyId: number, marketShopId: number) => {
  const { comnanyGoodsExcitationRuleId } = formData
  QueryBaseRule({
    comnanyGoodsExcitationRuleId: comnanyGoodsExcitationRuleId!,
    companyId,
    marketShopId
  }).then(res => {
    if (res.code === 200) {
      ...
      formData.ladderCalcType = res.data.ladderCalcType
      formData.goodsExcitationRuleLadderInputList =
      ...
      formData.operGoodsDetailInputList = res.data.goodsUsedRuleOutputs.map(
...
const onSelected = (val: OperGoodsDetailInput[]) => {
  formData.operGoodsDetailInputList = val.map(item => {
    ...
    if (checkedGoodsIds.includes(item.goodsId)) {
      item['checked'] = true
    } else {
      item['checked'] = false
    }
...
const toggleDialogStatus = (
  ...
  if (!dialogShow.value) {
    ...
    if (comnanyGoodsExcitationRuleId && companyId) {
      // 编辑操作
      formData.comnanyGoodsExcitationRuleId = comnanyGoodsExcitationRuleId
