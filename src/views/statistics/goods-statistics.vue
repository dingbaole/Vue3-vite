<template>
  <el-card shadow="never">
    <el-form :inline="true" :model="formData" :rules="rules" ref="ruleFormRef">
      ...
      <el-form-item label="订单编号" prop="orderNo">
        <el-input v-model="formData.orderNo" placeholder="订单编号" clearable />
      </el-form-item>
      <el-form-item label="SKU编号" prop="skuNo">
        <el-input v-model="formData.skuNo" placeholder="SKU编号" clearable />
      </el-form-item>
      ...
      <el-form-item label="关联sap销售单号" prop="extOrderNo">
        <el-input
          v-model="formData.extOrderNo"
          placeholder="关联sap销售单号"
          clearable
        />
      </el-form-item>
      <el-form-item label="销售员姓名/手机号" prop="promoterId2">
        <el-select
          v-model="formData.promoterId2"
          filterable
          clearable
          remote
          reserve-keyword
        ...
        >
          <el-option
            ...
            :key="item.promoterId"
            :label="`${item.mobileNo}(${item.promoterName})`"
            :value="item.promoterId"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="SAP帐号名" prop="promoterId3">
        <el-select
          v-model="formData.promoterId3"
          filterable
          clearable
          remote
          reserve-keyword
          placeholder="请输入SAP帐号名"
          :remote-method="remoteMethod3"
          :loading="selectRemote3.loading"
          style="width: 200px"
        >
          <el-option
            v-for="item in selectRemote3.options"
            :key="item.promoterId"
            :label="item.marketPromoterNo"
            :value="item.promoterId"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="订单来源" prop="platformId">
        ...
        >
          <el-option
            v-for="item in promoterTypeList"
            :label="item.name"
            :value="item.id"
            :key="item.id"
          />
      ...
      <el-form-item>
        <el-button
          v-permission="'excitation:rule:stat-goods-list'"
          type="primary"
          @click="search"
  ...
  <el-card shadow="never" style="margin-top: 10px">
    <el-button
      v-permission="'excitation:rule:stat-goods-asyn-export'"
      type="primary"
      @click="exportData"
    ...
    >
      ...
      <el-table-column label="订单来源" width="100" align="center">
        <template #default="{ row }">
          <!-- <div v-if="row.platformId === Promoter.PLATFORM">平台推广员</div> -->
          <div v-if="row.platformId === Promoter.MARKET">集市</div>
          <div v-if="row.platformId === Promoter.SAPECC">SAP-ECC</div>
          <div v-if="row.platformId === Promoter.SAPS4">SAP-S4</div>
        </template>
      </el-table-column>
      <el-table-column
        label="订单编号"
      ...
      >
        <template #default="scope">
          <div>{{ scope.row.orderNo ? scope.row.orderNo : '--' }}</div>
          <div v-if="scope.row.orderDirection === 12">源: {{ scope.row.relateOrderNo ? scope.row.relateOrderNo : '--' }}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="关联sap销售单号"
      ...
      >
        <template #default="{ row }">
          <span>{{ formatStr(row.orderInvoiceTime) }}</span>
        </template>
      </el-table-column>
...
const genParams = () => {
  if (formData.promoterId2 && formData.promoterId3) {
    return null
  }
  formData.promoterId = formData.promoterId2
    ? formData.promoterId2
    : formData.promoterId3
    ? formData.promoterId3
    : undefined
  if (!dateRange.value) {
    return {
...
const exportData = async () => {
  await ruleFormRef.value.validate(valid => {
    if (valid) {
      ...
      GoodsListExport({ ...params }).then(res => {
