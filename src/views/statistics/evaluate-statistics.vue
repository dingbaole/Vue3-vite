<template>
  <el-card shadow="never">
    <el-form :inline="true" :model="formData" :rules="rules" ref="ruleFormRef">
      ...
      <el-form-item>
        <el-button
          v-permission="'excitation:rule:stat-evaluate-list'"
          type="primary"
          @click="search"
  ...
  <el-card shadow="never" style="margin-top: 10px">
    <el-button
      v-permission="'excitation:rule:stat-evaluate-asyn-export'"
      type="primary"
      @click="exportData"
    ...
    >
      ...
      <el-table-column label="服务激励金额" property="baseAmount" />
      <el-table-column label="客户评价星级" property="evaluateStar" />
      <el-table-column
        label="评价激励系数"
        property="ruleCommissionValue"
        width="140"
        align="center"
      >
        <template #default="scope">
          <span>{{ scope.row.ruleCommissionValue }}%</span>
        </template>
      </el-table-column>
      <el-table-column label="评价激励金额" property="commissionAmount" />
      <el-table-column label="员工姓名" property="empName" />
      <el-table-column label="员工手机号" property="mobileNo" />
      <el-table-column
        label="评价激励时间"
        property="calculateTime"
        width="160"
      >
        <template #default="scope">
          <span>
            {{ dayjs(scope.row.calculateTime).format('YYYY-MM-DD HH:mm:ss') }}
          </span>
        </template>
      </el-table-column>
      <el-table-column
...
import { EvaStatisticsList, EvaStatisticsExport } from '@/webapi/statistics'
import type { Evaluate } from '@/webapi/statistics'
...
const dateRange = ref([])
const formData = reactive<Evaluate>({
  cityId: undefined,
  companyId: undefined,
  orderNo: '',
...
const exportData = async () => {
  await ruleFormRef.value.validate(valid => {
    if (valid) {
      ...
      EvaStatisticsExport({ ...params }).then(res => {
